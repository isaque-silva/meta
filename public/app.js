    ...
    ? `
      ...
      <div class="meta-meses">
            ${meses.map(mm => {
              ...
              const saldoFixo = Number(mm.valor_atual) || 0;
              const ded = Math.max(0, Number(mm.valor_deduzido || 0));
              const dedFixo = Math.max(0, Number(mm.valor_deduzido_fixo || 0));
              const dedVar = Math.max(0, Number(mm.valor_deduzido_variavel || 0));
              ...
              const varSaldo = Math.max(0, Math.round((ganhoVar - dedVar) * 100) / 100);
              const saldoTotal = Math.max(0, Math.round((saldoFixo + varSaldo) * 100) / 100);
              const alvoTotal = Math.max(0, Math.round((alvo + ganhoVar) * 100) / 100);
              const pctMes = alvoTotal > 0 ? Math.min(100, Math.max(0, (saldoTotal / alvoTotal) * 100)) : 0;
              const chipMes = ded > 0
                ...
                <article class="meta-mes-card">
                  ...
                  <div class="meta-mes-stats">
                    ...
                    <div class="meta-mes-stat ded">
                      ...
                      <span class="val">${ded > 0 ? (isOperador() ? fmtPctGlobal(alvoTotal > 0 ? (ded / alvoTotal) * 100 : 0) : '−' + fmtBRL(ded)) : '—'}</span>
                    </div>
                    <div class="meta-mes-stat saldo">
                      <span class="lbl">Saldo</span>
                      <span class="val">${isOperador() ? fmtPctGlobal(pctMes) : fmtBRL(saldoTotal)}</span>
                    </div>
                  </div>
                  <div class="meta-mes-foot">
                    ${footDed}
                    <span>${isOperador()
                      ? `Saldo ${fmtPctGlobal(pctMes)} do mês`
                      : `Meta fixa ${fmtBRL(alvo)}${ganhoVar > 0 ? ` + variável ${fmtBRL(ganhoVar)}` : ''} → restam ${fmtBRL(saldoTotal)} (fixo ${fmtBRL(saldoFixo)}${ganhoVar > 0 ? ` + var ${fmtBRL(varSaldo)}` : ''})`
                    }</span>
                  </div>
                </article>`;
            }).join('')}
...
window.deduzirMeta = async id => {
  ...
  const opcoesMes = meses.length
    ? meses.map(mm => {
        const off = Number(mm.mes_offset);
        const sel = off === defaultOffset ? ' selected' : '';
        const saldoZero = Number(mm.valor_atual) <= 0 && (Number(mm.valor_variaveis || mm.valor_melhorias || 0) - Number(mm.valor_deduzido_variavel || 0)) <= 0;
        const aviso = saldoZero ? ' (saldo zerado)' : '';
        const alvoFixo = Number(mm.valor_inicial) || 0;
        const saldoFixo = Number(mm.valor_atual) || 0;
        const varTotal = Math.max(0, Number(mm.valor_melhorias || mm.valor_variaveis || 0));
        const varDed = Math.max(0, Number(mm.valor_deduzido_variavel || 0));
        const varSaldo = Math.max(0, Math.round((varTotal - varDed) * 100) / 100);
        const saldoTotal = Math.max(0, Math.round((saldoFixo + varSaldo) * 100) / 100);
        const pctSaldo = alvoFixo > 0 ? (saldoTotal / alvoFixo) * 100 : 0;
        return `<option value="${off}"${sel}>Mês ${off + 1} · ${mesLabelCurto(mm.data_mes)} — ${isOperador() ? `saldo ${fmtPctGlobal(pctSaldo)}` : `meta fixa ${fmtBRL(alvoFixo)} · saldo ${fmtBRL(saldoTotal)}`}${aviso}</option>`;
      }).join('')
    : '';
  ...
  function atualizarPreview() {
    ...
    const alvoMes = Number(mm.valor_inicial) || 0; // meta fixa (base do percentual)
    const saldoFixo = Number(mm.valor_atual) || 0;
    const varTotal = Math.max(0, Number(mm.valor_melhorias || mm.valor_variaveis || 0));
    const varDed = Math.max(0, Number(mm.valor_deduzido_variavel || 0));
    const varSaldo = Math.max(0, Math.round((varTotal - varDed) * 100) / 100);
    const saldoTotal = Math.max(0, Math.round((saldoFixo + varSaldo) * 100) / 100);
    if (mesHintEl) {
      mesHintEl.innerHTML = saldoTotal <= 0
        ? '<span style="color:var(--warn)">Saldo deste mês já está zerado; a dedução não reduzirá mais o saldo.</span>'
        : (isOperador()
          ? `Alvo do mês: <b>100%</b> · Saldo atual: <b>${fmtPctGlobal(alvoMes > 0 ? (saldoTotal / alvoMes) * 100 : 0)}</b>`
          : `Meta fixa: <b>${fmtBRL(alvoMes)}</b> · Saldo: <b>${fmtBRL(saldoTotal)}</b> (fixo ${fmtBRL(saldoFixo)} + var ${fmtBRL(varSaldo)})`);
    }
    const p = Number(pctInput.value);
    if (alvoMes > 0 && p > 0) {
      const saldoPct = alvoMes > 0 ? (saldoTotal / alvoMes) * 100 : 0;
      ...
      calcEl.innerHTML = isOperador()
        ...
        : (() => {
            ...
            const cap = Math.min(v, saldoTotal);
            const ex = v > saldoTotal && saldoTotal > 0
  ...
  document.getElementById('d-save').onclick = async () => {
    ...
    const varSaldo = Math.max(0, Math.round((varTotal - varDed) * 100) / 100);
    const saldoTotal = Math.max(0, Math.round((saldoFixo + varSaldo) * 100) / 100);
    const saldoPctDisponivel = alvoMes > 0 ? (saldoTotal / alvoMes) * 100 : 0;
    if (saldoTotal <= 0 || saldoPctDisponivel <= 0) {
    ...
    if (p > saldoPctDisponivel) {
      ...
      const disponivelTxt = isOperador()
        ...
        : `${fmtPctGlobal(saldoPctDisponivel, 2)} (${fmtBRL(saldoTotal)})`;
